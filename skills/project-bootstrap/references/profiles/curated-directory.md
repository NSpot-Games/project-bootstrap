...
| `<project>/docs/design/example-<instance>.md` | one complete, real entry in the actual schema with every provenance field filled; a walkthrough of it through inclusion, review and publication; the pages it appears on; a numbered "schema gaps" section recording every gap the entry exposed; questions the entry raised beyond schema gaps go to `<project>/docs/OPEN-QUESTIONS.md`, with a one-line pointer here | bootstrap, before the data model and the site docs are finished |
