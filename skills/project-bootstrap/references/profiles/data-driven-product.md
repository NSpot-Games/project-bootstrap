...
| `<project>/docs/design/<runtime>.md` | the one rule that must never be broken, stated first; a numbered pipeline diagram; exact state shapes; deterministic tables where behaviour is tunable; the contract with any external component as a schema; validation and fallback; telemetry event names | bootstrap |
| `<project>/docs/design/architecture.md` | repo layout (packages, apps, backend); one line per module; the key interfaces as code; data flow for the main operation with a latency budget; persistence; offline and failure behaviour; testing strategy and CI budget (`references/core/economy.md §3`); security | bootstrap |
...
| `<project>/docs/design/example-<instance>.md` | a complete, real instance of the data model in the actual schema; a walkthrough of how the instance plays out against the core loop; a check that every cross-reference in the instance resolves; a pointer to the numbered *Gaps* section of the instance's README under the C0 example location, where every gap the instance exposed is recorded (`SKILL.md §3`) — this doc is the walkthrough, the folder is the data | bootstrap, before the runtime or architecture docs are finished |
